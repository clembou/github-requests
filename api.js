...
module.exports = function(app) {
  ...
  // App end points
  app.get('/projects', /*passport.authenticate('oauth-bearer', { session: false }), */ function(req, res) {
    res.json(appData);
  });

  app.get('/authenticate/github/:code', /* passport.authenticate('oauth-bearer', { session: false }), */ function(req, res) {
  ...
  app.get('/repos/:organisation/:repo/issues', /*passport.authenticate('oauth-bearer', { session: false }), */function(req, res) {
  ...
  app.get('/repositories/:repoId/issues', /* passport.authenticate('oauth-bearer', { session: false }), */ function(req, res) {
  ...
  app.get('/repos/:organisation/:repo/issues/:issueId', /* passport.authenticate('oauth-bearer', { session: false }), */ function(req, res) {
    if (!validateRepository(req.params.organisation, req.params.repo, appData.projects))
      res.status(403).send({ error: 'Invalid repository name' });
  ...
  app.post('/repos/:organisation/:repo/issues', /* passport.authenticate('oauth-bearer', { session: false }), */ function(req, res) {
    if (!validateRepository(req.params.organisation, req.params.repo, appData.projects))
      res.status(403).send({ error: 'Invalid repository name' });
  ...
  app.get('/repos/:organisation/:repo/issues/:issueId/comments', /* passport.authenticate('oauth-bearer', { session: false }), */ function(req, res) {
    if (!validateRepository(req.params.organisation, req.params.repo, appData.projects))
      res.status(403).send({ error: 'Invalid repository name' });
  ...
  app.post('/repos/:organisation/:repo/issues/:issueId/comments', /* passport.authenticate('oauth-bearer', { session: false }), */ function(req, res) {
    if (!validateRepository(req.params.organisation, req.params.repo, appData.projects))
      res.status(403).send({ error: 'Invalid repository name' });
  ...
  function trySendEmailsForGitHubWebHook(req, res)  {
    ...
    console.log(`Request app: Email notification starting ${email}, ${subject}, ${content}`);
  ...
  app.post('/github-webhooks', [bodyParser.json({verify:verifyRequest})], function(req, res) {
