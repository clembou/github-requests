...
const titleFormatter = (cell, row, enumObject, index) => {
  return <IssueInfo title={row.title} labels={row.labels} url={row.url} />;
};
...
class TeamIssues extends Component {
  ...
  getTeamRepos() {
    ghClient.gh.getTeam(this.props.match.params.teamId).listRepos().then(resp => {
    ...
    Promise
      .all(
        this.state.repos.map(repo => {
          return ghClient.gh.getIssues(this.props.match.params.organisation, repo.name).listIssues();
        })
      )
      .then(responses => {
  ...
  render() {
    const options = {
      ...
      sizePerPageList: [
        ...
        {
          text: 'All',
          value: this.state.issues.length
        }
