...
const Issue = props => {
  ...
  const header = (
    <span>
      {props.issue.title}
      {' '}by{' '}
      {getCreator(props.issue).name || getCreator(props.issue).login}
      {' '}
      {githubLink}
      {' '}
      <Label className="panel-title pull-right" bsStyle={props.issue.state === 'open' ? 'primary' : 'danger'}>{props.issue.state}</Label>
    </span>
  );
  return (
    <Panel header={header} eventKey={props.issue.id}>
